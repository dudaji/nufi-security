...
## 1. 정책 변경 감사 (누가·언제·무엇)
로그: `/home/dudaji/work/nufi/security/samples/sla/policy_changes.jsonl`  ·  총 5건  ·  체인: **OK** (5건)
...
| 시각 | 목적지 | 프로세스 | 심각도 |
|---|---|---|---|
| 2026-06-23T11:30:00+0900 | api.anthropic.com |  | high |

## 4. 점검항목 커버리지 (control coverage)
카탈로그 v1.2  ·  **직접 25**(충족 25/미충족 0)  ·  **부분 10**  ·  **범위밖 13**
...
| 규제 | 직접(충족/미충족) | 부분 | 범위밖 |
|---|---|---|---|
| 금융분야 AI 보안 안내서 (fsec-ai) | 4 (4/0) | 5 | 5 |
| 망분리 보안대책 (net-sep) | 5 (5/0) | 0 | 0 |
| 개인정보보호법 (pipa) | 6 (6/0) | 2 | 2 |
| 신용정보법 (cia) | 5 (5/0) | 1 | 2 |
| ISMS-P 인증기준 (isms-p) | 5 (5/0) | 2 | 4 |

### 금융분야 AI 보안 안내서 (fsec-ai) — 직접 4(충족 4/미충족 0)·부분 5·범위밖 5
...
| C-11 | 안내서 점검 ⑪·32~35 | 실시간 모니터링·정기점검·보고 | 감사 결정 모니터링 + 정책 변경 보고(compliance report 산출) | ✅ 충족 | decisions.total=5; policy_change_audit.records#=5 |
...
### 개인정보보호법 (pipa) — 직접 6(충족 6/미충족 0)·부분 2·범위밖 2
| 항목 | 출처 | 요구사항 | NuFi 통제 | 충족 | 증빙/보강 |
|---|---|---|---|---|---|
...
| PIPA-15 | 개인정보보호법 §15(수집·이용) | 개인정보 수집·이용 동의 기반 처리 | 범위밖 — 동의 수집·관리는 절차적 요건(게이트웨이 비범위) | ⛔ 범위밖 | — |
| PIPA-34 | 개인정보보호법 §34(유출 통지) | 개인정보 유출 시 정보주체·당국 통지 | 감사 알림 파이프라인(부분) — 법적 통지 절차·서식 미구현 | 🟡 부분충족 | P2 — 유출 통지 자동화(법적 서식 연동) |

### 신용정보법 (cia) — 직접 5(충족 5/미충족 0)·부분 1·범위밖 2
| 항목 | 출처 | 요구사항 | NuFi 통제 | 충족 | 증빙/보강 |
|---|---|---|---|---|---|
...
| CIA-19-INTEG (←M-2.7) | 신용정보법 §19(전산시스템 안전보호 — 위변조방지) | 신용정보전산시스템 위변조방지 | 감사·변경 해시체인 무결성 | ✅ 충족 | integrity_ok=True |
| CIA-19-IDS | 신용정보법 §19(전산시스템 안전보호 — 침입탐지·접근통제) | 신용정보전산시스템 침입탐지·접근통제 | 침입탐지(IDS) 미구현 — RBAC(부분) | 🟡 부분충족 | P3 — 침입탐지·자산 RBAC |
| CIA-33-2 | 신용정보법 §33-2(전송요구권) | 개인신용정보 전송요구권(마이데이터) | 범위밖 — 데이터 포터빌리티는 게이트웨이 비범위 | ⛔ 범위밖 | — |
| CIA-20-2 | 신용정보법 §20-2(관리·보호인력) | 개인신용정보 관리·보호 인력 지정 | 범위밖 — 조직·인력 운영은 절차적 요건(게이트웨이 비범위) | ⛔ 범위밖 | — |

### ISMS-P 인증기준 (isms-p) — 직접 5(충족 5/미충족 0)·부분 2·범위밖 4
| 항목 | 출처 | 요구사항 | NuFi 통제 | 충족 | 증빙/보강 |
|---|---|---|---|---|---|
...
| ISMS-2.5 | ISMS-P 2.5 | 인증 및 권한관리 | 세션 RBAC(viewer/operator) — 자산·사용자 인증 미구현 | 🟡 부분충족 | P1+ — 자산 RBAC·인증 체계 |
| ISMS-2.7 | ISMS-P 2.7 | 암호화 적용(전송·저장) | 범위밖 — TLS 종단은 인프라 레이어(게이트웨이 비범위) | ⛔ 범위밖 | — |
| ISMS-3.5 | ISMS-P 3.5 | 정보주체 권리보호(열람·정정·삭제·처리정지) | 범위밖 — 정보주체 권리 이행은 절차적 요건(게이트웨이 비범위) | ⛔ 범위밖 | — |
