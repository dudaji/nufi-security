...
| 무엇을 하려나 | 문서 / 명령 |
|---|---|
| **처음이라 손으로 감 잡기** — 토이 프로젝트("환불 도우미") 하나를 SDK 전환 + `nufi-egress` CLI 운영으로 끝까지 따라하기(20~30분·root/네트워크 불필요) | **[`docs/HANDS_ON.md`](docs/HANDS_ON.md)** |
...
| **v0.0.5 정책 운영 자동화 데모** (Operate · B1) — 다중 프로파일·경로별 묶기·무재기동 되돌리기·변경 감사(root 불필요) | `./scripts/demo_policy_ops.sh` → 매뉴얼 [`docs/OPS_POLICY_AT_SCALE.md`](docs/OPS_POLICY_AT_SCALE.md) (`nufi-egress policy …`) |
...
| **CLI 레퍼런스** — `nufi-egress` 전 서브커맨드(플래그·예시·종료코드) | [`docs/CLI.md`](docs/CLI.md) — `nufi-egress {render,apply,disable,status,feedback,doctor,coverage,monitor,init,audit,targets,flow-tap,policy}` |
