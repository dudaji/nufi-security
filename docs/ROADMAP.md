...
- **현재 상태 (v1.2 카탈로그):** PIPA 10항목(6 direct/2 partial/2 oos) · CIA 7항목(4/1/2) · ISMS-P 11항목(5/2/4). 모든 direct 항목의 eval 규칙이 기존 증빙으로 자동 판정 검증 완료.
...
유지: 게이트웨이 코어(경량)
차기 옵션: LLM 라우팅 레이어(M) · 한국어 가드레일(L)
```
