...
수용 기준(binary):
- [ ] private 기본 라우팅 + public 폴백 동작.
- [ ] public 모델로 나간 요청 **100%가 감사 로그에 적재**(요청 본문·모델·타임스탬프·결정).
- [ ] 설정으로 private/public 백엔드·라우팅 규칙 변경 가능.
...
수용 기준(binary):
- [ ] 샘플셋에서 한국 PII(주민번호·사업자번호·전화·계좌·카드·여권) **정규식+체크섬** 탐지.
- [ ] 한국어 인명/지명 **KoELECTRA NER** 탐지(정규식 미탐 보완).
- [ ] API 키/자격증명 **detect-secrets** 탐지.
- [ ] 정책에 따라 block / redact / pseudonymize / warn 동작 + 결정 로그.
