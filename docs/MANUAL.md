...
### 7.5 테넌트/RBAC 거부 — 종료코드 3

읽기전용(`viewer`) 역할로 **정책 변경**(bind/snapshot/rollback)이나 **다테넌트 집계**
(`--all-tenants`)를 시도하면 접근이 거부되고 **종료코드 3**으로 끊깁니다. 테넌트 경계 자체는
viewer·operator 모두에서 유지됩니다.

```bash
# viewer 가 정책을 바꾸려 하면 → 거부, exit 3
nufi-egress report --all-tenants --role viewer ...    # AccessDenied → exit 3
nufi-egress report --all-tenants --role operator ...  # operator 면 통과
```

역할 모델과 테넌트 격리는 [`MULTITENANCY.md`](MULTITENANCY.md) 가 권위입니다.
...
> 종료코드 요약: **0** 정상 · **1** 무결성/리포트 게이트 실패 · **3** 권한(RBAC) 거부.
...
| **테넌트(tenant)** | 격리 경계(예: `tenant:acme`). 테넌트별로 조회가 격리되어 한 테넌트가 다른 테넌트의 감사 데이터를 보지 못한다. |
| **RBAC(역할 기반 접근 제어)** | 역할: `viewer`(조회 전용) / `operator`(조회 + 정책 변경 + 다테넌트 집계). viewer 가 권한 밖 작업을 하면 종료코드 3으로 거부. |
