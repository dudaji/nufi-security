# retain_raw 보존정책 & Vault 키 회전 가이드 (CMP-124 · v0.0.2 M2·D2)

> **승인:** 보드 2abc6285 (Option A). **스코프:** 운영/설정·문서·검증. 신규 egress
> 차단 규칙이나 게이트웨이 결정 로직 변경 없음.
> **오설정 방지 검증:** `tests/test_cmp124_reload.py::test_misconfig_prevention_retain_raw_and_kek_rotation`

이 문서는 고객 보안검토 1순위 항목 두 가지 — (1) egress 원문(raw) 보존(retain_raw)과
...
- public `retain_raw: true` 전환은 **반드시** 아래를 선행한다:
  ...
  3. CPO 리뷰 → CEO 정렬(거버넌스). 코드 변경이 아니라 **별도 설정 승인**.
...
이 확장은 별도 이슈+보드 승인 대상(키관리 동작 변경). 본 CMP-124 범위는 절차 문서화까지다.
...
## 3. 운영 체크리스트 (요약)
- [ ] public `retain_raw` 는 `false` 인가? `true` 라면 접근통제+보존기간+CPO/CEO 정렬이 끝났는가?
