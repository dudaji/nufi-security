...
주소(KR_LOCATION)는 모델 백엔드가 어휘밖·구조적 주소(신도시·도로명·상세주소 등)를
놓칠 수 있어, 환경변수 `M5_LOCATION_UNION=1` 로 **주소 채널만 모델 ∪ 확장 규칙**
유니온을 켜면 규칙이 그 놓침을 회복합니다(인명·기타 채널·오탐 불변, 재현율 ≥ 각
백엔드 단독). gazetteer 백엔드에는 규칙이 이미 있어 무효과입니다.
