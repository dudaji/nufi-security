...
python3 -m enforcement.cli init strict-kr-pii --out ./config

# 적용 전 결정 미리보기(파일 미생성)
python3 -m enforcement.cli init audit-only --dry-run
```

> **CLI 표기 규약.** 본 가이드의 모든 운영 명령은 **통합 CLI** `python3 -m enforcement.cli <서브커맨드>`
> (`render·apply·disable·status·feedback·doctor·coverage·monitor·init` 단일 진입점)로 통일합니다.
> 패키지를 설치하면 동일 명령을 `nufi-egress <서브커맨드>` 로 줄여 부를 수 있습니다(설치형 console_scripts
> 패키징은 [CMP-141] 에서 제공 예정 — 그 전까지는 위 `python3 -m enforcement.cli` 형이 정식 표기).
> 레거시 모듈별 진입점(`python3 -m egress_audit.init_cli`)은 **비설치 환경 동등 실행**으로만 남기며,
> 신규 사용은 통합 CLI 를 권장합니다.
...
```bash
python3 -m enforcement.cli doctor          # 사람읽기 + JSON  (통합 CLI · 설치 시 nufi-egress doctor)
python3 -m enforcement.cli doctor --json   # 기계용 JSON 만
#   (비설치 동등 실행: python3 -m enforcement.doctor — 신규 사용은 위 통합 CLI 형 권장)
```
...
> 이 절의 감사·로그조회 명령(`audit_bot`·위 heredoc)을 통합 CLI 서브커맨드(`nufi-egress audit …`)로
> 편입하는 작업은 코드 선행이 필요해 [CMP-141] 에서 진행합니다 — 머지 후 본 절을 통합 CLI 표기로 갱신합니다.
> 그 전까지는 위 모듈 직접 실행 형태가 정식 표기입니다.
...
python3 -m enforcement.cli init audit-only --out ./config
