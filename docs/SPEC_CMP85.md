...
### 라우팅·취합 모델 (CMP-92 CPO 확정 — 2026-06-24)

보드가 CMP-89 데모를 리뷰하고 시나리오 모델 변경을 요청했다("사용자는 그냥 LLM을 호출, 내부 파이프라인이 private/public을 결정·호출·결과 취합"). 거버넌스상 시나리오=SPEC §P3 설계(CPO 소유). CPO 확정:
