...
| **v0.4.16-patch49** | MANUAL §3 감사 로그 스키마 | egress_audit.jsonl 레코드 필드 표·JSON 예시 추가 |
| **v0.4.16-patch48** | ROADMAP 달성 수치 표 | v0.4.16 기준 10개 지표 목표/달성 대비 표 추가 |
...
| ~~한국어 생성형 가드레일(프롬프트 인젝션·탈옥)~~ | ✅ 완료 | v0.4.17 (patch60~80) — 인젝션 탐지기·정책·벤치·게이트웨이/LiteLLM 통합 |

### 완료된 과제 (v0.3.0~v0.4.16 에서 해소)

| 과제 | 해소 버전 |
|---|---|
| 한국어 프롬프트 인젝션 탐지·정책·게이트웨이 차단 | v0.4.17 (patch60~80) — 직접/간접 인젝션 패턴, 심각도 4단계, 벤치마크 골드셋 |
| 파일/디렉터리 PII 스캔 + CI/pre-commit 통합 | v0.4.17 (patch83~86) — scan, .nufiignore, SARIF 2.1.0 출력 |
| 텍스트 변환 CLI (mask·redact·explain) | v0.4.17 (patch116~126) — 탐지 이유 설명, 마스킹, 리다크션 |
| 프로젝트 초기화·감시·diff 스캔 | v0.4.17 (patch90~104) — init, watch, diff, config validate |
...
`scripts/check_docs.py` 회귀 가드가 향후 드리프트를 기계로 막습니다. **501 테스트** · **20 데모** PASS.
CLI 서브커맨드: **23개**. 내부 링크 수: 145개+ (v0.4.17-patch128 기준). 문서 교차링크 체계 완성.
