...
*작성 시점: 2026-07-03 · 대상: v0.4.6 (main, origin 동기화됨)*
...
| **v0.4.5** | 운영자 매뉴얼 갱신 | MANUAL.md 에 SDK·PII 라우팅·강건성·벤치마크 반영, 정확도 수치 갱신 |
| **v0.4.4** | 데모 정합성 | 데모 assertion 갱신 + uvicorn 미설치 graceful SKIP → FAIL 0 |
| **v0.4.3** | 문서 품질 강화 | ops 제외 안내 전파 + CLI 누락 보완 + 릴리스 노트 확장 |
| **v0.4.2** | 게이트웨이 강건성 | 탐지 타임아웃(fail-closed)·프롬프트 크기 제한·지연 추적·방어 파싱 |
...
### 완료된 과제 (v0.3.0~v0.4.6 에서 해소)

| 과제 | 해소 버전 |
|---|---|
| 인명(KR_PERSON) 정확도 — Wilson 하한 0.85→0.91+ 달성 | v0.3.0 (성씨 사전 확장+규칙∪NER 유니온) |
...
모든 공개 정확도·지연 수치(재현율 0.977, 정밀도 0.9948, p95 41ms, KR_LOCATION Wilson 하한
0.9417, KR_PERSON 0.9516/0.9106)가 커밋된 리포트 JSON 과 일치하며, `scripts/check_docs.py`
회귀 가드가 향후 드리프트를 기계로 막습니다.
